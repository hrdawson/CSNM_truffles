...
truffle.data = read.csv("raw_data/wvos_trufflecounts_allsites.csv", na.strings = c("", "NA")) |> # Fill in the file path
  ...
  filter(site != "PIS_2019") |>
  #filter(site != "HZL_CTRL") |>
  #filter(site != "HZL_2022") |>
  filter(genus != "Rhizopogon") |>
  filter(genus != "Endogone") |>
  ...
  #add unique species determined by sequence data
  #read in species determination sheet
  trufflespecies.data = read.csv("unique_species.csv", na.strings = c("", "NA"))
  #left join using the sample id column
  truffle.data = dplyr::left_join(truffle.data, trufflespecies.data, by = "sample_id")
...
##Show all unique species IDs
#species.unique <- truffle.data %>%
  #group_by(species_determination) %>%
  #summarize(count = n_distinct(species_determination))
...
trufflecounts.hazeldell.all = truffle.data |>
    filter(site %in% c("HZL_2022", "HZL_CTRL")) |>
    group_by(species_guess) |>
    summarize(ct.all = length(species_guess))

trufflecounts.hazeldell.burn = truffle.data |>
  # Filter to just hazeldell
  filter(site %in% c("HZL_2022", "HZL_CTRL")) |>
  # Tell R to group all the truffles by genus
  group_by(species_guess, burn_history) |>
  # and to count how many instances are of each genus
  summarize(ct.burn = length(species_guess))

trufflecounts.hazeldell = trufflecounts.hazeldell.burn |>
  left_join(trufflecounts.hazeldell.all)
...
#trufflecounts = truffle.data |>
  # Tell R to group all the truffles by genus
  #group_by(genus, burn_history) |>
  # and to count how many instances are of each genus
  #summarize(ct = length(genus))

ggplot(trufflecounts.hazeldell, # This is the dataframe with your data
       ...
       aes(x = reorder(species_guess, -ct.all), y = ct.burn, fill = burn_history)) +
      xlab("Species") +
      ylab("Count") +
      ggtitle("Truffle counts in burned and unburned \noak habitat") +
